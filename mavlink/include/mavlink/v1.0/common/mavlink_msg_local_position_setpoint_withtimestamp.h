...
#define MAVLINK_MSG_ID_LOCAL_POSITION_SETPOINT_LEN 21
#define MAVLINK_MSG_ID_51_LEN 21
...
#define MAVLINK_MESSAGE_INFO_LOCAL_POSITION_SETPOINT { \
	...
	6, \
	{  { "time_boot_ms", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_local_position_setpoint_t, time_boot_ms) }, \
...
static inline uint16_t mavlink_msg_local_position_setpoint_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t coordinate_frame, uint32_t time_boot_ms, float x, float y, float z, float yaw)
{
	...
	_mav_put_uint8_t(buf,20, coordinate_frame);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 21, 223);
}
						           ...
						           uint8_t coordinate_frame,uint32_t time_boot_ms,float x,float y,float z,float yaw)
{
	...
	msg->msgid = MAVLINK_MSG_ID_LOCAL_POSITION_SETPOINT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 21, 223);
}
...
{
	return mavlink_msg_local_position_setpoint_pack(system_id, component_id, msg, local_position_setpoint->coordinate_frame, local_position_setpoint->time_boot_ms, local_position_setpoint->x, local_position_setpoint->y, local_position_setpoint->z, local_position_setpoint->yaw);
}
...
static inline void mavlink_msg_local_position_setpoint_send(mavlink_channel_t chan, uint8_t coordinate_frame, uint32_t time_boot_ms, float x, float y, float z, float yaw)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[21];
	_mav_put_uint32_t(buf, 0, time_boot_ms);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LOCAL_POSITION_SETPOINT, buf, 21, 223);
#else
	mavlink_local_position_setpoint_t packet;
	packet.time_boot_ms = time_boot_ms;
	packet.x = x;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_LOCAL_POSITION_SETPOINT, (const char *)&packet, 21, 223);
#endif
}
...
#else
	memcpy(local_position_setpoint, _MAV_PAYLOAD(msg), 21);
#endif
}
